...
export async function POST(request: Request) {
  try {
    ...
    if (type === 'posts') {
      ...
      while (tweets.length < fetchCount) {
        const remaining = fetchCount - tweets.length;
        const pageSize = Math.min(remaining, 100); // Max 100 per request

        const url = `${TWITTER_API_BASE}/users/${userId}/tweets?max_results=${pageSize}&exclude=retweets,replies&tweet.fields=created_at,public_metrics,attachments,entities&expansions=attachments.media_keys&media.fields=type,url,preview_image_url,width,height,alt_text${nextToken ? `&pagination_token=${nextToken}` : ''}`;
        ...
        if (pageTweets.length === 0) {
          console.log(`Pagination stopped: No more tweets available. Total fetched: ${tweets.length}`);
          break; // No more tweets available
        }

        tweets = tweets.concat(pageTweets);
        console.log(`Fetched page with ${pageTweets.length} tweets. Total so far: ${tweets.length}/${fetchCount}`);
        ...
        if (!nextToken) {
          console.log(`Pagination stopped: No next_token. Total fetched: ${tweets.length}`);
          break; // No more pages
        }
      }

      console.log(`Final result: Fetched ${tweets.length} out of ${fetchCount} requested posts`);
      ...
      while (!fetchedEnough && totalFetched < 3200) { // Safety limit
        ...
        // Filter for replies and group by date
        const pageReplies = allTweets.filter((tweet: TwitterTweet) => tweet.in_reply_to_user_id);
